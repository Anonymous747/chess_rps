...
flutter test \
  ...
  --machine \
  --coverage \
  -- coverage-path $PACKAGE_PACKAGE_LCOV_INFO_PATH > $PACKAGE_TEST_REPORT_PATH

escapedPath="$(echo $PACKAGE_PATH | sed '/\//\\\//g')"

if [[ "$OSTYPE" =~^darwin ]]; then
  gsed -i "s/^SF:lib/SF:$escapedPath\/lib/g" $PACKAGE_LCOV_INFO_PATH
else
  sed -i "s/^SF:lib/SF:$escapedPath\/lib/g" $PACKAGE_PACKAGE_LCOV_INFO_PATH
fi